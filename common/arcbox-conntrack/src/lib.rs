...
//!
//! Stateful connection tracking (Swiss-table conntrack with a fast-path cache),
//! zero-copy in-place packet translation (SNAT and reverse NAT with RFC 1624
//! incremental checksums), and the [`HostNetIO`] egress seam. Pure — depends
//! only on the packet and datapath primitives, with no VM, VirtIO, or I/O
//! coupling. Extracted from `arcbox-net` so non-VM consumers can reuse the NAT
//! datapath.
//!
//! # Performance Targets
//!
