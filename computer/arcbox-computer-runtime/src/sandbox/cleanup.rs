...
// The release bodies live with the machine that names them
// (`Effect::SpawnRelease`); this module still drives them until R3 PR-F2
// flips the manager onto the actor.
use super::*;
use crate::lifecycle::tasks::release::release_everything;
...
pub(super) async fn remove_sandbox_impl(
    ...
    release_everything(id, expected, network, config, cow_manager, snapshots).await?;
