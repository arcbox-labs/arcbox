...
/// Root directory for ArcBox shell integration files (profile data dir).
...
async fn status(format: OutputFormat) -> Result<()> {
    ...
    let profile_injected = if let Some(ref p) = profile {
        check_profile_injected(p, detected_shell).await
    } else {
        false
    };
...
async fn write_shell_init_scripts(shell_dir: &Path) -> Result<()> {
    ...
    let zsh = format!(
        r#"# ArcBox shell integration (zsh)
# This file is auto-generated by `abctl setup install`.
export PATH="{}:${{PATH}}"
fpath+=("{}")
"#,
        bin_dir.display(),
        completions_dir.join("zsh").display()
    );

    let bash = format!(
        r#"# ArcBox shell integration (bash)
# This file is auto-generated by `abctl setup install`.
export PATH="{}:${{PATH}}"
...
"#,
        bin_dir.display(),
        completions_dir.join("bash").display()
    );

    let fish = format!(
        r#"# ArcBox shell integration (fish)
# This file is auto-generated by `abctl setup install`.
fish_add_path -gP "{}"
for f in "{}"/*.fish
    source $f 2>/dev/null
...
"#,
        bin_dir.display(),
        completions_dir.join("fish").display()
    );
...
fn source_line(shell_dir: &Path, shell: ShellKind) -> String {
    match shell {
        ShellKind::Zsh => {
            format!(
                "{PROFILE_MARKER}\nsource \"{}\" 2>/dev/null || :",
                shell_dir.join("init.zsh").display()
            )
        }
        ShellKind::Bash => {
            format!(
                "{PROFILE_MARKER}\nsource \"{}\" 2>/dev/null || :",
                shell_dir.join("init.bash").display()
            )
        }
        ShellKind::Fish => {
            format!(
                "{PROFILE_MARKER}\nsource \"{}\" 2>/dev/null; or true",
                shell_dir.join("init.fish").display()
            )
        }
    }
}

async fn check_profile_injected(path: &Path, shell: ShellKind) -> bool {
    check_profile_injected_with_shell_dir(path, &shell_dir(), shell).await
}

async fn check_profile_injected_with_shell_dir(
    path: &Path,
    shell_dir: &Path,
    shell: ShellKind,
) -> bool {
    let snippet = source_line(shell_dir, shell);
    tokio::fs::read_to_string(path)
        .await
        .is_ok_and(|content| content.contains(&snippet))
}
...
async fn inject_profile(shell: ShellKind) -> Result<Option<PathBuf>> {
    ...
    if check_profile_injected(&path, shell).await {
        return Ok(Some(path));
    }
...
async fn remove_profile_injection(shell: ShellKind) -> Result<Option<PathBuf>> {
    ...
    // Remove only the injection block for the currently selected profile.
    let snippet = source_line(&shell_dir(), shell);
    ...
    while index < lines.len() {
        if snippet_lines.len() == 2
            && lines[index] == snippet_lines[0]
            && lines
                .get(index + 1)
                .is_some_and(|line| *line == snippet_lines[1])
        {
            index += 2;
            continue;
        }
...
/// Links Docker tools into the profile's `bin/` (user-space, no root).
///
...
///
/// Searches xbin (app bundle) first, then the profile's `runtime/bin/` (daemon).
async fn link_docker_tools_to_user_bin(abctl_exe: &Path, user_bin: &Path) -> usize {
    ...
    // 2. Runtime bin (populated by daemon).
    let runtime_bin = HostLayout::from_env_or_default()
        .data_dir
        .join("runtime/bin");
...
mod tests {
    ...
    fn source_lines_contain_marker() {
        let shell_dir = PathBuf::from("/tmp/arcbox-profile/shell");
        for shell in [ShellKind::Zsh, ShellKind::Bash, ShellKind::Fish] {
            let line = source_line(&shell_dir, shell);
            assert!(line.contains(PROFILE_MARKER));
            assert!(line.contains("/tmp/arcbox-profile/shell/init."));
        }
    }
    ...
    async fn profile_injection_is_idempotent() {
        ...
        let snippet = source_line(&dir.path().join("shell"), ShellKind::Zsh);
        ...
        assert!(check_profile_injected_with_shell_dir(&profile, &shell_dir, ShellKind::Zsh).await);
