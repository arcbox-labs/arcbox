...
import { ErrorCode, ErrorInfoSchema } from '../src/gen/arcbox/sandbox/v1/errors_pb.js';
import {
  ArcBoxError,
  CapabilityError,
  CommandFailedError,
...
function daemonError(code: ErrorCode, suggestion = '', context: Record<string, string> = {}) {
...
describe('toArcBoxError', () => {
  it('maps ErrorInfo details to the registry class with code, suggestion, and context', () => {
    const err = toArcBoxError(
      daemonError(ErrorCode.SANDBOX_NOT_FOUND, 'list sandboxes with `abctl sandbox list`', {
        id: 'sb-1',
      }),
      'sandbox.info',
    );
  ...
  it('maps state and capability registry codes', () => {
    expect(toArcBoxError(daemonError(ErrorCode.SANDBOX_PAUSED), 'op')).toBeInstanceOf(
      SandboxStateError,
    );
    expect(toArcBoxError(daemonError(ErrorCode.NESTED_VIRT_UNSUPPORTED), 'op')).toBeInstanceOf(
      CapabilityError,
    );
    expect(toArcBoxError(daemonError(ErrorCode.COMMAND_TIMEOUT), 'op')).toBeInstanceOf(
      CommandTimeoutError,
    );
    expect(toArcBoxError(daemonError(ErrorCode.FILE_NOT_FOUND), 'op')).toBeInstanceOf(
      FileNotFoundError,
    );
  });
  ...
  it('routes on the Connect code when no ErrorInfo detail rode along', () => {
    const notFound = toArcBoxError(new ConnectError('gone', Code.NotFound), 'op');
    expect(notFound).toBeInstanceOf(NotFoundError);
    expect(notFound.code).toBe('NotFound');

    const deadline = toArcBoxError(new ConnectError('late', Code.DeadlineExceeded), 'op');
    expect(deadline).toBeInstanceOf(RequestTimeoutError);
    expect(deadline.suggestion).toContain('requestTimeoutMs');

    expect(toArcBoxError(new ConnectError('bad', Code.InvalidArgument), 'op')).toBeInstanceOf(
      InvalidArgumentError,
    );
  });

  it('turns a missing socket (ENOENT) into ConnectionFailedError with a daemon-start suggestion', () => {
    const enoent = Object.assign(new Error('connect ENOENT /tmp/x.sock'), { code: 'ENOENT' });
...
describe('CommandFailedError', () => {
  it('names the exit code and carries the result', () => {
    const err = new CommandFailedError({ exitCode: 2, stdout: '', stderr: 'oops' });
