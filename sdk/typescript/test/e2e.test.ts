...
describe.skipIf(!enabled)('hello world against a live daemon', () => {
  it('creates a sandbox, moves files, runs commands, and cleans up', async () => {
    ...
    try {
      ...
      expect(await sandbox.files.readText('/tmp/hello.txt')).toBe('hello from arcbox\n');
      ...
      const bg = await sandbox.commands.run('for i in 1 2 3; do echo line$i; done', {
        background: true,
      });
      let streamed = '';
      ...
      const sleeper = await sandbox.commands.run(['/bin/sleep', '300'], { background: true });
