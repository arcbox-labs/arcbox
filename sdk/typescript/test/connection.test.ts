...
describe('resolveConnection', () => {
  it('defaults to <home>/.arcbox/run/arcbox.sock', () => {
    ...
    expect(conn.socketPath).toBe(join(homedir(), '.arcbox', 'run', 'arcbox.sock'));
  ...
  it('rejects socketPath together with apiUrl', () => {
    expect(() =>
      resolveConnection({ socketPath: '/tmp/x.sock', apiUrl: 'https://cloud.example' }, {}),
    ).toThrow(InvalidArgumentError);
  });

  it('resolves the api key from option over environment', () => {
    expect(resolveConnection({ apiKey: 'opt' }, { ARCBOX_API_KEY: 'env' }).apiKey).toBe('opt');
    expect(resolveConnection({}, { ARCBOX_API_KEY: 'env' }).apiKey).toBe('env');
  });

  it('treats empty environment strings as unset', () => {
    const conn = resolveConnection({}, { ARCBOX_API_URL: '', ARCBOX_SOCKET: '' });
    expect(conn.baseUrl).toBe(UDS_BASE_URL);
    expect(conn.socketPath).toBe(join(homedir(), '.arcbox', 'run', 'arcbox.sock'));
  });

  it('carries requestTimeoutMs through', () => {
    expect(resolveConnection({ requestTimeoutMs: 5000 }, {}).requestTimeoutMs).toBe(5000);
  });
});
