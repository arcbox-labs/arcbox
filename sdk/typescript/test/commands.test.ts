...
import { ArcBoxError, CommandFailedError, SandboxDiedError } from '../src/errors.js';
...
describe('commandResultFromExecution', () => {
  ...
  it('throws SandboxDiedError when the execution ended without an observed exit', () => {
    const execution = create(ExecutionSchema, { id: 'x', error: 'session broke' });
    expect(() => commandResultFromExecution(execution, '', '')).toThrow(SandboxDiedError);
  });

  it('throws on a terminal execution with no exit status and no error', () => {
    const execution = create(ExecutionSchema, { id: 'x' });
    expect(() => commandResultFromExecution(execution, '', '')).toThrow(ArcBoxError);
  });
});
