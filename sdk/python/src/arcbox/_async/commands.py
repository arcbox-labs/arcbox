...
class AsyncOutputStream:
    ...
    def __init__(self, chunks: AsyncGenerator[OutputChunk]) -> None:
        self._chunks = chunks
    ...
    async def __aenter__(self) -> AsyncOutputStream:
        return self
...
class AsyncCommandHandle:
    ...
    @property
    def output(self) -> AsyncOutputStream:
        ...
        with wrap_errors("commands.wait_for_exit"):
            while True:
                ...
                if remaining is None:
                    execution = await self._wait_slice(_WAIT_SLICE_SECONDS)
                elif remaining >= 1:
                    execution = await self._wait_slice(min(_WAIT_SLICE_SECONDS, int(remaining)))
                else:
                    ...
                    # check above raises.
                    execution = await self._wait_slice(0)
    ...
    async def wait_for_log(
        self, pattern: str | re.Pattern[str], timeout: float | None = None
    ) -> str:
    ...
    async def _wait_slice(self, slice_seconds: int) -> process_pb2.Execution:
    ...
    async def write_stdin(self, data: str | bytes) -> None:
        ...
        with wrap_errors("commands.write_stdin"):
            offset = await self._stdin_cursor()
            status = await self._client.unary(
                _PROCESS + "WriteStdin",
    ...
    async def stdin_status(self) -> StdinStatus:
        ...
        with wrap_errors("commands.stdin_status"):
            status = await self._fetch_stdin_status()
    ...
    async def _stdin_cursor(self) -> int:
        """The tracked stdin cursor, resynced from the daemon when unknown."""
        offset = self._stdin_offset
        if offset is None:
            offset = int((await self._fetch_stdin_status()).bytes_written)
        return offset

    async def _fetch_stdin_status(self) -> process_pb2.StdinStatus:
        status = await self._client.unary(
...
class AsyncCommands:
    ...
    async def run(
        ...
        *,
        ...
        if isinstance(stdin_data, bytes):
            await self._feed_stdin(handle, stdin_data)
        if background:
            return handle
        result = await handle.wait_for_exit()
        return result.expect() if check else result

    async def get(self, command_id: str) -> AsyncCommandHandle:
    ...
    async def _feed_stdin(self, handle: AsyncCommandHandle, data: bytes) -> None:
