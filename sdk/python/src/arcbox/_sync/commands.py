...
class CommandHandle:
    ...
    def _attach_events(
    ...
    ) -> Generator[process_pb2.ExecutionEvent]:
        ...
        with wrap_errors(operation):
            ...
            while True:
                ...
                try:
                    with self._attach(stdout_offset, stderr_offset, read_timeout) as stream:
                        for event in stream:
                            kind = event.WhichOneof("event")
                            if kind == "output":
                                chunk = event.output
    ...
    def wait_for_log(self, pattern: str | re.Pattern[str], timeout: float | None = None) -> str:
        ...
        with wrap_errors("commands.wait_for_log"):
            try:
                with closing(self._attach_events("commands.wait_for_log", deadline)) as events:
                    for event in events:
